...
let MemDescription1: Member =
    { Member.name = "Troels"
      Member.description = ("29456661", 1994, [ Interests.Jazz ]) }
...
let MemDescription3 =
    { Member.name = "Morten"
      Member.description = ("29456660", 1995, [ Interests.CS; Interests.Golf ]) }
...
let extractTargetGroup (p: Predicate, r: Register) =
    let res =
        List.map (fun item -> item.description) r
